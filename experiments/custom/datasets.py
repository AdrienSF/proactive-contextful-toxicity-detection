...
def add_team_player_tokens(messages, players, teams, relative_to=-1) -> str:
  # for consistency, assign player and team nums according to order of last appearance in chat
  unique_players = sorted(list(set(players)), key=lambda x: list(reversed(players)).index(x))
  unique_teams = sorted(list(set(teams)), key=lambda x: list(reversed(teams)).index(x))


  return '\n'.join(['[Player' + str(unique_players.index(players[i])) + ']' + '[Team' + str(unique_teams.index(teams[i])) + ']' + ' ' + messages[i] for i in range(len(messages))])

def current_player_only_team_player_tokens(messages, players=None, teams=None, relative_to=None):
    # just put [Team0][Player0] everywhere
    return '\n'.join(['[Team0][Player0] ' + m for m in messages])
